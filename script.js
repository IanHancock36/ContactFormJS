...
form.addEventListener("submit", (event) => {
  let messages = []; // this means empty array. this is = to all error messages
  ...
  if (email.value.length <= 5) {
    messages.push("Email must be longer than that.");
  };
  if (phoneNumber.value.length <= 10) {
    messages.push("Please Enter A Correct Phone Number");
  }
